...
import { observable, makeObservable, IComputedValue, action } from "mobx";
...
import isAddHelmRepoDialogOpenInjectable from "./is-open.injectable";
...
class NonInjectedAddHelmRepoDialog extends React.Component<AddHelmRepoDialogProps & Dependencies> {
  ...
  render() {
    const { isOpen, close, onOpen, ...dialogProps } = this.props;
...
export const AddHelmRepoDialog = withInjectables<Dependencies, AddHelmRepoDialogProps>(NonInjectedAddHelmRepoDialog, {
  getProps: (di, props) => ({
    ...
    isOpen: di.inject(isAddHelmRepoDialogOpenInjectable),
